...
ValueLens is an institutional-grade equity intelligence website. Every night it scans the S&P 500, Nasdaq 100, S&P 400, and Russell 1000, runs each candidate through an AI fundamental analysis pipeline (reverse DCF, balance-sheet "zombie" detection, short-interest/momentum context), and surfaces ranked opportunities before the US market opens — alongside live insider buy tracking, an earnings-catalyst predictor, and a $100k virtual portfolio that auto-trades every AI pick so the system's calls can be tracked transparently over time.
...
| 🟢 **Insider Tracking** | Monitors SEC Form 4 filings for C-suite open-market buys. Activity per ticker accumulates over time rather than resetting on each detection, with a sortable table and a treemap/heatmap comparison view of the largest buys, plus full transaction detail (insider name, role, shares, value) on demand. |
...
* `scanner.py`: The core screening + deep-value pipeline, target-price scaling, and Russell 1000/index universe sourcing.
